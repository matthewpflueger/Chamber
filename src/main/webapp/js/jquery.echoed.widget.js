...
Echoed = {
    ...
    Urls:{
        Api: "http://localhost.com:8080/api/",
        Css: "http://localhost.com:8080/css/",
        Echo: "http://localhost.com:8080/echo/"
    },
    Models:{},
    Collections:{},
    init: function(options) {
        this.partnerId = options.partnerId;
        var opener = new Echoed.Views.Components.Opener({ el: '#echoed-opener', EvAg: EventAggregator });
...
Echoed.Views.Components.InfiniteScroll = Backbone.View.extend({
    initialize: function(options){
        ...
        this.element = $('<div id="echoed-infiniteScroll">Loading More Products</div>');
...
Echoed.Views.Pages.Exhibit = Backbone.View.extend({
    initialize: function(options){
        _.bindAll(this,'render','next','close','init');
        this.EvAg = options.EvAg;
        this.el = options.el;
        this.EvAg.bind('exhibit/open', this.init);
        this.EvAg.bind('infiniteScroll', this.next);
        this.element = $(this.el);
        this.element.append($("<div id='echoed-close'><img src='http://localhost.com:8080/images/btn_close_x.png' /></div><div id='echoed-e-c-b'><div id='echoed-exhibit'></div></div>"));
        this.fade = $("<div id='echoed-fade' ></div>");
        ...
        this.jsonUrl = Echoed.Urls.Api + "partner/" + Echoed.partnerId;
        this.nextInt = 1;
    },
    events: {
    ...
    render: function(data){
        ...
        if(data.partner){
            self.contentDescription = "Products recently purchased at " + data.partner.name;
    ...
    addTitle: function(title, image){
        var self = this;
        var titleDiv = $('<div></div>').addClass('item_wrap').addClass('no_filter').attr("id","echoed-title");
...
Echoed.Views.Components.Product = Backbone.View.extend({
    ...
    render: function(){
        this.template = '<div class="item_content"><img class="exhibit_image" width="230px" src=""/><div class="item_text"></div></div>';
        ...
        this.el.addClass("item_wrap").addClass('Brand-' + encodeURIComponent(this.model.get("echoBrand"))).addClass('Browse-' + encodeURIComponent(this.model.get("echoCategory"))).html(this.template).attr("href", landingUrl);
        var img = this.el.find("img");
        var text = this.el.find(".item_text");
        this.el.attr('href', Echoed.Urls.Echo + this.el.attr("id"));
...
$(document).ready(function(){
    ...
    cssNode.href = Echoed.Urls.Css + "echoed-widget.css";
    document.getElementsByTagName('HEAD')[0].appendChild(cssNode);

    var fontNode = document.createElement("link");
    fontNode.type= "text/css";
    fontNode.rel = "stylesheet";
    fontNode.href= "https://fonts.googleapis.com/css?family=Lato:300,400,700";
    ...
    window.onload = function(){
        ...
        var echoedContainer = document.createElement('div');
        echoedContainer.id = "echoed-exhibit-container";
        echoedContainer.style.display = "none";
        body.appendChild(echoedContainer);

        Echoed.init({ partnerId : "a676139c-1259-4b3e-b5dd-60bac13c0b0f"});
    }

...
class EchoedUserServiceActor(
    ...
    def handle = {
        ...
        case msg: AssignTwitterService =>
            ...
            def assign() {
                log.debug("Assigning TwitterService to {}", echoedUser)
                msg.twitterService.assignEchoedUser(echoedUser.id).onSuccess {
                    ...
                    case AEUR(_, Right(twitterUser)) =>
                        ...
                        log.debug("Assigned {} to {}", twitterUser, echoedUser)
                }.onFailure { case e => error(e) }
            }

            try {
                log.debug("Attempting to TwitterService to EchoedUser {}", this.echoedUser.id)
                ...
                msg.facebookService.assignEchoedUser(this.echoedUser).onComplete(_.fold(
                    ...
                    _ match {
                        ...
                        case AEUR(_, Right(fu)) =>
                            log.debug("Assigning FacebookService to EchoedUser {}", echoedUser.id)
                            ...
                            log.debug("Assigned FacebookUser {} to EchoedUser {}", fu.facebookId, echoedUser.id)
                    }))
            }

            try {
                log.debug("Attempting to assign FacebookService to EchoedUser {}", echoedUser.id)
                    ...
                    ep => {
                        if(ep.isEchoed) {
                            ...
                            },
                            {
                                ...
                                log.warning("Did not find PartnerSettings {} for Echo {}", ep.partnerSettingsId, ep.id)
                            })
                    },
                    {
                        channel ! EchoToResponse(msg, Left(EchoedUserException("Invalid echo possibility")))
                        log.debug("Did not find Echo {}", msg.echoPossibilityId)
                    })
            } catch {
                case e =>
        ...
        case msg @ ScatterResponse(
            ...
            def sendResponse(responses: List[Message]) {
                ...
                responses.foreach(message => message match {
                    ...
                    case unknown => log.error("Error in responses from echo scatter {}", unknown)
                })
            ...
            try {
                ...
                either.fold(
                    error => {
                        ...
                        log.error("Received error response: {}, {}", error, error)
                    },
                                ...
                                fs.echo(echo, em).onComplete(_.fold(
                                    ...
                                    _ match {
                                        ...
                                        case FETFR(_, Right(fp)) =>
                                            ...
                                            log.debug("Successfully echoed {} to {}", ec, fp)
                                    }))
                        })),
                    {
                        channel ! ETFR(msg, Left(EchoedUserException("Not associated to Facebook")))
                        log.debug("Could not echo {} because user {} does not have a FacebookService", echo, echoedUser)
                    })
            } catch {
                case e => error(e)
            }
                                ...
                                fs.publishAction(action, obj, objUrl).onComplete(_.fold(
                                    ...
                                    _ match {
                                        ...
                                        case PublishActionToFacebookResponse(_, Right(fa)) =>
                                            ...
                                            log.debug("Successfully published Action")
                                    }))
                        })),
                    {
                        channel ! PublishFacebookActionResponse(msg, Left(EchoedUserException("Not Associated to Facebook")))
                        log.debug("Could not publish action {} because user {} does not have a FacebookService", action)
                    })
            } catch {
                case e => error(e)
            }            
                            ...
                            case GetTwitterServiceWithIdResponse(_, Right(ts)) => ts.tweet(echo, em).onComplete(_.fold(
                                ...
                                _ match {
                                    ...
                                    case TweetResponse(_, Right(twitterStatus)) =>
                                        ...
                                        log.debug("Successfully tweeted {} to {}", ec, ts)
                                }
                            ))
                        })),
                    {
                        channel ! EchoToTwitterResponse(msg, Left(EchoedUserException("Not associated to Twitter")))
                        log.debug("User {} not associated to Twitter to echo {}", echoedUser, echo)
                    })
            } catch {
                case e => error(e)
            }
        ...
        case GetFriendsResponse(_, Left(e)) =>
            log.error("Received error finding friends for EchoedUser {}: {}", echoedUser.id, e)
        ...
        case msg: FetchFacebookFriends =>
            ...
            Option(echoedUser.facebookUserId).cata(
                facebookServiceLocator.locateById(_).onComplete(_.fold(
                    log.error("Could not get FacebookService for {}: {}", echoedUser, _),
                    _ match {
                        case LocateByIdResponse(_, Left(e)) =>
                            log.error("Could not get FacebookService for {}, {}", echoedUser, e)
                        case LocateByIdResponse(_, Right(ac: ActorClient)) =>
                            ac.actorRef.!('_fetchFacebookFriends)(me)
                    }
                )),
               log.debug("No FacebookService for EchoedUser {}", echoedUser.id)
            )


        case GetFollowersResponse(_, Left(e)) =>
            log.error("Received error fetching followers for EchoedUser {}, {}", echoedUser.id, e)
        ...
        case msg: FetchTwitterFollowers =>
            ...
            Option(echoedUser.twitterUserId).cata(
                twitterServiceLocator.getTwitterServiceWithId(_).onComplete(_.fold(
                    log.error("Could not get TwitterService for {}, {}", echoedUser, _),
                    _ match {
                        case GetTwitterServiceWithIdResponse(_, Left(e)) =>
                            log.error("Could not get TwitterService for {}: {}", echoedUser, e)
                        case GetTwitterServiceWithIdResponse(_, Right(ac: ActorClient)) =>
                            log.debug("Got TwitterService for {}", echoedUser.id)
                            ac.actorRef.!(GetFollowers())(me)
                    }
                )),
                log.debug("No TwitterService for EchoedUser {}", echoedUser.id)
            )

...
class TwitterServiceActor(twitterAccess: TwitterAccess,
                          ...
                          echoClickUrl: String,
                          var twitterUser: TwitterUser) extends EchoedActor {
    ...
    private def updateTwitterStatus(ts: TwitterStatus, me: ActorRef, retries: Int = 0) {
        ...
        def scheduleRetry(e: Throwable) {
            if (retries >= maxUpdateTwitterStatusRetries) {
                log.error("Failed to update Twitter status {}, retries {}, last error was {}", ts, retries, e)
            } else {
                log.debug("Retrying {} due to error {}", ts, e)
                context.system.scheduler.scheduleOnce(updateTwitterStatusRetryInterval milliseconds, me, RetryTweet(ts, retries + 1))
            }
        }

        log.debug("Trying to update Twitter status {}, retries {}", ts, retries)
        twitterAccess.updateStatus(
            ...
            _ match {
                ...
                case UpdateStatusResponse(_, Right(twitterStatus)) =>
                    ...
                    log.debug("Successfully tweeted {}", tw)
            }))
    }
    ...
    def handle = {
        ...
        case msg @ Tweet(echo, message) =>
            ...
            try {
                log.debug("Creating new TwitterStatus with message {} for {}", echo, message)
        ...
        case msg @ Logout(twitterUserId) =>
            ...
            try {
                ...
                self ! PoisonPill
                log.debug("Logged out Twitter user {}", twitterUserId)
            } catch {
                case e =>
